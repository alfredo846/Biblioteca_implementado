...
<div class="container-fluid p-0">
    ...
    <div class="col-12 bg-primario rounded-top mb-3">
        <div class="col-12 py-4">
            <h1 class="text-white fw-bold text-uppercase text-center fs-4 mb-2">Control de Autores</h1>
            </div>
    </div>
    <!-- Container Slogan -->
    ...
    <div class="row w-100 h-100 m-0 p-0">
        ...
        <!-- Header Section -->
        <form action="{{ route('autores.index') }}" method="POST" class="row w-100 p-0 mx-auto align-items-center" id="form-number-items">
            ...
            <div class="col-12 col-md-4">
                <a href="{{ route('autores.create') }}" class="btn btn-primario text-white btn-sm d-block mx-auto w-75">Crear Nuevo</a>
            </div>
        </form>
        <!-- Header Section -->
                ...
                <table class="table table-primaria d-none d-lg-table text-center align-center">
                    ...
                    <tbody id="">
                        ...
                        <tr class="d-none d-lg-table-row col-12">
                            <td class="col-2">{{$item->nombre_a}}</td>
                            <td class="col-2">{{$item->nacionalidad_a}}</td>
                            <td class="col-2">{{date('d-m-Y', $date = strtotime($item->fecha_n))}}</td>
                            @if ($item->fecha_d == null)
                                <td class="col-2">No tiene</td>
                            @else
                                <td class="col-2">{{$item->fecha_d}}</td>
                            @endif
                            <td class="col-2">
                                <span @class([
                              ...
                              {{--  start botón de editar --}}
                                <div class="row justify-content-evenly w-100">
                                    <div class="col text-center px-0">
                                        <a href="{{ route('autores.edit', $item) }}" class="btn btn-sm btn-primario text-white w-20" >
                                    ...
                                    <div class="col text-center px-0">
                                        <form action="{{ route('autores.toggle.status', $item) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                    ...
                                    <div class="col text-center px-0">
                                        @if(!$item->activo)
                                        <form action="{{ route('autores.destroy', $item) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="card shadow-lg my-3 py-3 mx-auto d-lg-none">
            <div class="card-body text-center">
                <p>Nombre: {{ $item->nombre_a }}</p>
                <p>Nacionalidad: {{ $item->nacionalidad_a }}</p>
                <p>Fecha Nacimiento: {{ date('d-m-Y', $date = strtotime($item->fecha_n)) }}</p>
                <p>Fecha Defución: {{ $item->fecha_d }}</p>
                @if($item->activo)
            ...
            <div class="card-footer">
                <div class="row">
                    <div class="col text-center">
                        <a href="{{ route('autores.edit', $item) }}" class="btn btn-sm btn-primario text-white">Editar</a>
                    </div>
                    <div class="col text-center">
                        <form action="{{ route('autores.toggle.status', $item) }}" method="POST">
                                            @csrf
                                            ...
                                            <input type="submit" @if($item->activo)
                                                value="Desactivar"
                                                @else
                                                value="Activar"
                                                @endif
