    ...
    <div class="container-fluid p-0">
        ...
        <div class="col-12 bg-primario rounded-top mb-3">
            <div class="col-12 py-4">
                <h1 class="text-white fw-bold text-uppercase text-center fs-4 mb-2">Control de Prestamos</h1>
            </div>
        </div>
        <!-- Container Slogan -->
        ...
        <div class="row w-100 h-100 m-0 p-0">
            ...
            <!-- Header Section -->
            <form action="{{ route('prestamos.index') }}" method="POST" class="row w-100 p-0 mx-auto align-items-center"
                        ...
                        <tbody id="">
                            @forelse($items as $item)
                                <tr class="d-none d-lg-table-row col-12">
                                    ...
                                    <td class="col-1">{{ $item->user. ' '. $item->ap. ' '. $item->am }}</td>
                                    <td class="col-2">{{ date('d-m-Y', $date = strtotime($item->fecha_p)) }}</td>
                                    <td class="col-1">{{ $item->observacion_p }}</td>
                                    <td class="col-1">{{ date('d-m-Y', $date = strtotime($item->fecha_dev))}}</td>
                                    <td class="col-1">{{ $item->observacion_dev }}</td>
                                    <td class="col-1">{{ $item->lib }}</td>
                                    ...
                                    @else
                                        <td class="col-1">
                                            <p class="badge bg-danger">Prestado</p>

                                        </td>
                                    @endif
                                    <td class="col-1">{{ $item->tipo_p }}</td>
                                    <td class="col-1">
                                        <span @class([
                                    ...
                                    <td class="align-center col-2">
                                        ...
                                        <div class="row justify-content-evenly w-100">
                                            <div class="col text-center px-0">
                                                <a href="{{ route('prestamos.edit', $item) }}"
                                            ...
                                            <div class="col text-center px-0">
                                                <form action="{{ route('prestamos.toggle.status', $item) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
...
@endif
                                            ...
                                            <div class="col text-center px-0">
                                                @if (!$item->activo)
                                                    <form action="{{ route('prestamos.destroy', $item) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
            ...
            @foreach ($items as $item)
                ...
                <div class="card shadow-lg my-3 py-3 mx-auto d-lg-none">
                    ...
                    <div class="card-body text-center">
                        <p>{{ $item->user. ' '. $item->ap. ' '. $item->am  }}</p>
                        <p>{{ date('d-m-Y', $date = strtotime($item->fecha_p)) }}</p>
                        <p>{{ $item->observacion_p }}</p>
                        <p>{{ date('d-m-Y', $date = strtotime($item->fecha_dev)) }}</p>
                        <p>{{ $item->observacion_dev }}</p>
                        <p>{{ $item->lib }}</p>
                        <p>{{ $item->tipo_p }}</p>
                    ...
                    <div class="card-footer">
                        <div class="row">
                            ...
                            <div class="col text-center">
                                <form action="{{ route('prestamos.toggle.status', $item) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="activo"
