        ...
        <div class="row justify-content-center">
            ...
            <div class="col-12 bg-primario rounded-top mb-3">
                <div class="col-12 py-4">
                    <h1 class="text-white fw-bold text-uppercase text-center fs-4 mb-2">Crear Nuevo Tema</h1>
                   </div>
            </div>
            <!-- Container Slogan -->

            <!-- Form -->
            <form action="{{ route('temas.store') }}" class="form" method="POST">
                @csrf
                @method('POST')
                    ...
                    <div class="row justify-content-center">
                        <div class="col-12 col-lg-10">
                            ...
                            <div class="form-floating my-2">
                                <input
                                    @class(['form-control'=> !$errors->first('nombre_t'), 'form-control is-invalid' => $errors->first('nombre_t')])
                                    autocomplete="off"
                                    type="text"
                                    value="{{ old( 'nombre_t' ) }}"
                                    name="nombre_t" {{-- <-- Nombre del Campo --}}
                                    placeholder="input-text"
                                    id="input-text"
                                    required>
                                <label for="input-text">Nombre del tema</label>

                                @if($errors->first('nombre_t'))
                                <div class="invalid-feedback">
                                    <i>{{ $errors->first('nombre_t') }}</i>
                                </div>
                                @endif
                ...
                <div class="d-flex justify-content-end mt-5">
                    <a title="Regresar" href="{{ route('temas.index') }}" class="text-end fs-6 text-secundario"><img src="{{ asset('img/regresa.jpg')}}" width="30" height="30"></a>
                </div>
            </form>
            <!-- Form End -->
        </div>
